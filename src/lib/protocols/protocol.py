...
from lib.message import Message, Initiate, Established, Inack, \
    MessageClassFactory, Decoder
import logging
...
class Protocol:
    ...
    def receive(client_socket: socket):
        message, serverAddress = \
            Protocol.decode_received_message(client_socket)
        return message, serverAddress

    @staticmethod
    def perform_client_side_handshake(connection):
        logging.info(f"{MSG.MSG_HANDSHAKE_STARTING}")
        connection.reset_timer()

        while True:
            message = Initiate(connection.transfer_type,
                               connection.protocol.CODE)
            Protocol.send_initiate(connection.socket,
                                   connection.destination_host,
                                   connection.destination_port,
                                   message)

            try:
                decoded_message, downloader_address = \
                    Protocol.decode_received_message(connection.socket)
            except TimeoutError:
                if connection.end_connection_flag.is_set():
                    exit()
                continue
            connection.reset_timer()
            if Protocol.verify_inack(decoded_message,
                                     connection.transfer_type,
                                     connection.protocol.CODE):
                break

        Protocol.send_established(connection.socket,
                                  downloader_address[0],
                                  downloader_address[1],
                                  connection.file_name)

        logging.info(f"{MSG.MSG_HANDSHAKE_COMPLETED}")
    ...
    def perform_server_side_handshake(server, first_message, client_address):
        ...
        Protocol.sendInack(dedicated_client_socket,
                           client_address,
                           first_message)

        dedicated_client_socket.settimeout(CONST.IDLE_TIMEOUT)
        try:
            established_message, client_address =\
                Protocol.decode_received_message(dedicated_client_socket)
        except TimeoutError:
            logging.info(f"{MSG.MSG_KEEP_ALIVE_TIMEOUT}")
            logging.warn(f"{MSG.MSG_ESTABLISHED_NOT_RECEIVED}")
            exit()

        if established_message.message_type != Message.ESTABLISHED:
            logging.debug(f"{MSG.MSG_IS_NOT_ESTABLISHED}")
            exit()
        logging.debug(
            f"{MSG.MSG_IS_ESTABLISHED} {established_message.filename}")

        connection = Connection(client_address,
                                first_message.transfer_type,
                                session_protocol,
                                established_message.filename)
    ...
    def send_initiate(socket, host, port, message):
        logging.info(f"{MSG.MSG_SENDING_INITIATE}")
    ...
    def process_initiate(first_message, client_address, server):
        if first_message.message_type != Message.INITIATE:
            logging.debug(f"{MSG.MSG_IS_NOT_INITIATE}")
            exit()
        if client_address in server.clients:
            logging.debug(f"{MSG.MSG_CLIENT_ALREADY_HAS_ASSIGNED_CONNECTION}")
            exit()
        logging.debug(f"{MSG.MSG_RECEIVED_INITIATE}")
        server.clients.append(client_address)

    @staticmethod
    def sendInack(dedicatedClientSocket, clientAddress, first_message):
        logging.debug(f"{MSG.MSG_SENDING_INACK}")

        message = Inack(first_message.transfer_type,
                        first_message.protocol_type)
        message_encoded = message.encode()
        dedicatedClientSocket.sendto(message_encoded, clientAddress)

        logging.debug(f"{MSG.MSG_SENT_INACK}")
        return
    ...
    def receive_inack(socket, host, port):
        ...
        if message_decoded.message_type != Message.INACK:
            logging.debug(f"{MSG.MSG_IS_NOT_INACK}")
            return

        logging.debug(f"{MSG.MSG_RECEIVED_INACK}")
    ...
    def verify_inack(message, transfer_type, protocol):
        ...
        if message.transfer_type != transfer_type:
            logging.debug(f"{MSG.MSG_TRANSFER_TYPE_NOT_MATCH}")
            return False
        if message.protocol_type != protocol:
            logging.debug(f"{MSG.MSG_PROTOCOL_NOT_MATCH}")
            return False
        logging.debug(f"{MSG.MSG_RECEIVED_INACK}")
        return True
    ...
    def send_established(socket, host, port, filename):
        logging.info(f"{MSG.MSG_SENDING_ESTABLISHED}")
        logging.debug(f"{MSG.MSG_FILE_NAME} {filename}")
    ...
    def decode_received_message(socket: socket):
        recv_buffer, clientAddress = socket.recvfrom(CONST.RECV_BUFFER_SIZE)
        ...
        decoded_message = Decoder.decode_after_fixed_header(message_type,
                                                            rest_of_message)
    ...
    def decode_message_from_buffer(buffer):
        ...
        rest_of_message = \
            buffer[:(message_class.MESSAGE_SIZE - Message.FIXED_HEADER_SIZE)]
        decoded_message = \
            Decoder.decode_after_fixed_header(message_type, rest_of_message)
        buffer = \
            buffer[(message_class.MESSAGE_SIZE - Message.FIXED_HEADER_SIZE):]
